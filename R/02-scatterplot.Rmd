...
A scatter plot displays two values (x,y) from continuous scales for each point, and can be used any time you want to show the relationship between two continuous variables. 

![Scatter plots](01-scatterplot_files/figure-gfm/graph_gallery-1.png)
*Legends and axis labels have been suppressed in the figures above in order to save space.*

Many popular bioinformatic visualizations are simply highly customized forms of scatter plot. The skills and tools covered in this chapter are generalizable to all scatter plots.
...
Like the previous section, this section makes use of ggplot2 and tidyverse libraries. In addition, we will use ggrepel, which extends the ggplot geom_label functionality.
...
For this section we will be using four data sources: expression data, annotations, MDS coordinates, and KEGG enrichment data.
...
Use point size in a scatter plot demonstrating the relationship between weight, cyl, and mpg in the mtcars data set. Consider using other aesthetics to add further information (e.g. gear, hp, or manufacturer).
...
```{r graph_gallery, echo=FALSE, eval=FALSE}
library(cowplot)
bottom.row <- plot_grid(t, r + guides(fill = "none", size = "none") + theme(axis.text.y = element_blank()))
top.row <- plot_grid(p + theme(axis.title = element_blank()) + guides(color = "none"),
                     s + theme(axis.title = element_blank()) + guides(color = "none"),
                     q + theme(axis.title = element_blank()) + guides(color = "none", shape = "none"),
                     nrow = 1, rel_widths = c(1, 2, 1.5))
plot_grid(top.row, bottom.row, ncol = 1)
```

Before closing out this section, download the .Rmd for the next section: box plots, violin plots, and marginal plots.
